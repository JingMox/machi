...
async fn walk_match(
    ...
    let meta = fs::metadata(dir).await.map_err(|e| {
        machi_tools::error::codes::execution(format!("stat {}: {e}", dir.display()))
    })?;
    if meta.is_file() {
    ...
    let mut rd = fs::read_dir(dir).await.map_err(|e| {
        machi_tools::error::codes::execution(format!("read_dir {}: {e}", dir.display()))
    })?;
        ...
        .map_err(|e| machi_tools::error::codes::execution(format!("read_dir next: {e}")))?
    {
        ...
        let ft = entry
            .file_type()
            .await
            .map_err(|e| machi_tools::error::codes::execution(format!("file_type: {e}")))?;
        if ft.is_dir() {
