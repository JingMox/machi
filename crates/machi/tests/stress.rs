...
mod stress {
    ...
    use machi::{
        DynTool, ErrorCode, InProcessHost, Journal, LlmSampler, MachiError, MockSampler,
    ...
    async fn parallel_64_readonly_tools() {
        ...
        let requests: Vec<_> = (0..64)
            .map(|i| machi::DispatchRequest {
                call: machi::ToolCall {
                    id: machi::ToolCallId::new(format!("c{i}")).expect("id"),
                    name: format!("t{i}"),
                    arguments: json!({}),
                },
            })
    ...
    async fn turn_path_records_into_prometheus() {
        use machi::{AgentBuilder, Session, TurnInput, TurnOptions, VecConversationState};
        ...
        assert!(text.contains("machi_turns_total"), "{text}");
