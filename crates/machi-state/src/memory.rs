...
pub trait MemoryPort: Send + Sync {
    ...
    async fn remember(&self, item: MemoryItem) -> Result<(), MachiError>;

    /// Fetch by exact key.
    async fn recall(&self, key: &str) -> Result<Option<MemoryItem>, MachiError>;

    /// List items (implementation-defined order); optional tag filter.
    async fn list(&self, tag: Option<&str>) -> Result<Vec<MemoryItem>, MachiError>;

    /// Delete by key; returns whether an entry existed.
    async fn forget(&self, key: &str) -> Result<bool, MachiError>;
}
...
impl MemoryPort for NullMemory {
    async fn remember(&self, _item: MemoryItem) -> Result<(), MachiError> {
        Ok(())
    }

    async fn recall(&self, _key: &str) -> Result<Option<MemoryItem>, MachiError> {
        Ok(None)
    }

    async fn list(&self, _tag: Option<&str>) -> Result<Vec<MemoryItem>, MachiError> {
        Ok(Vec::new())
    }

    async fn forget(&self, _key: &str) -> Result<bool, MachiError> {
        Ok(false)
    }
}
